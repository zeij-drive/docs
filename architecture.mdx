...
deobf-all (调度器)
  ...
  └─── P3 Skills（扩展工具链）
       ├── deobf-string                     （字符串反混淆）
       ├── deobf-indirect                   （间接跳转反混淆）
       ├── llvm-obfuscation                 （LLVM 混淆分析）
       ├── binary-lifting                   （二进制提升）
       ├── deobfuscating-javascript-malware （JS 恶意代码反混淆）
       ├── deobfuscating-powershell-obfuscated-malware （PowerShell 反混淆）
       ├── binary-analysis-patterns         （二进制分析模式）
       ├── yara-rule-authoring              （YARA 规则）
       ├── ghidra-headless                  （Ghidra 无头模式）
       ├── reverse-engineering-malware-with-ghidra（Ghidra 恶意代码逆向）
       ├── frida-17                         （Frida 动态插桩）
       └── radare2                          （radare2 逆向工具链）
```

## 调度流程

1. **激活**：用户调用 `/deobf-all` 或 `run_skill(name="deobf-all")`
2. **分层加载**：通过 `read_skill` 先加载 P0 核心 skill，再按需加载 P1/P2/P3 skill 到 agent 上下文
...
| 分层优先级 | P0/P1/P2/P3 四级分级，确保关键能力始终可用 |
